...
export default function ResetPasswordPage() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      if (!token) {
        feedbackWarning(showFeedback, "Reset token is missing.");
        return;
      }
      if (password.length < 8) {
        feedbackWarning(showFeedback, "Password must be at least 8 characters.");
        return;
      }
      if (password !== confirmPassword) {
        feedbackWarning(showFeedback, "Passwords do not match.");
        return;
      }
      ...
      feedbackSuccess(showFeedback, "Your password has been updated.", {
